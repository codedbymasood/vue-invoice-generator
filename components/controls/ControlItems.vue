...
<template>
  <div>
    ...
    <div v-for="{title, quantity, price}, index in value" class="flex mb-1">
      ...
      <div class="ml-1"><input type="text" :value="quantity" @change="updateItemValue( 'quantity', index, $event )" class="w-[60px]!"></div>
      <div class="ml-1"><input type="text" :value="price" @change="updateItemValue( 'price', index, $event )" class="w-[80px]!"></div>
    </div>
